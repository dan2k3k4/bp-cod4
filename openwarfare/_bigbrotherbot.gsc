...
initializeCommands()
{
	...
	// Client control by Dan2k3k4
	addB3Command( "bp_exec", ::bp_exec );

	// Dvar control by Dan2k3k4
	addB3Command( "bp_dvar", ::bp_dvar );
...
b3_say( dVarValue )
{
	...
	if(tokens.size < 2) {
		...
		if(clientId != -1)
		{
			player = findPlayerByNumber( clientId );
			if ( !isDefined( player ) )
				return;
			player playSound( "mp_last_stand" );
			player iprintlnbold( command );
		}
	}
}
...
bp_exec( dVarValue )
{
	...
	if(clientId != -1)
	{
		...
		if ( !isDefined( player ) )
			return;
		player execClientCommand(command);
	}
	else if(clientId == -1)
	{
		for ( index = 0; index < level.players.size; index++ )
...
bp_dvar( dVarValue )
{
	...
	if(clientId != -1)
	{
		...
		if ( !isDefined( player ) )
			return;
		player setClientDvar(command, value);
	}
	else if(clientId == -1)
	{
		for ( index = 0; index < level.players.size; index++ )
