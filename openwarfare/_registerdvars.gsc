...
init()
{
	...
	if ( level.oldschool ) {
		level.scr_jump_height = getdvarx( "scr_jump_height", "int", 64, 0, 2000 );
		level.scr_jump_slowdown_enable = getdvarx( "scr_jump_slowdown_enable", "int", 0, 0, 1 );
	} else {
		level.scr_jump_height = getdvarx( "scr_jump_height", "int", 39, 0, 2000 );
		level.scr_jump_slowdown_enable = getdvarx( "scr_jump_slowdown_enable", "int", 1, 0, 1 );
	}
	
	// BP Map Time Limit
	game["bp_maptimelimit"] = getdvarx( "bp_maptimelimit", "int", 60, 0, 180 );
	
	// BP Map Time Alpha
	//game["bp_maptime_alpha"] = getdvarx( "bp_maptime_alpha", "int", 1, 0, 1 );
	
	// BP Nade Cook
	level.scr_nade_cook_enable = getdvarx( "scr_nade_cook_enable", "int", 0, 0, 1 );
	...
	if ( level.oldschool ) {
		level.scr_fallDamageMinHeight = getdvarx( "scr_fallDamageMinHeight", "int", 256, 50, 9999 );
		level.scr_fallDamageMaxHeight = getdvarx( "scr_fallDamageMaxHeight", "int", 512, 50, 9999 );
	} else {
		level.scr_fallDamageMinHeight = getdvarx( "scr_fallDamageMinHeight", "int", 128, 50, 9999 );
		level.scr_fallDamageMaxHeight = getdvarx( "scr_fallDamageMaxHeight", "int", 300, 50, 9999 );
	}
