...
//		thread delayedLeaderDialogBothTeams( "obj_lost", oldTeam, "obj_taken", team );
	...
	if ( getTeamFlagCount( team ) == level.flags.size ) {
		if ( level.scr_dom_secured_all_bonus_time > 0 ) {
			...
			level.securedAllBonusTime = openwarfare\_timer::getTimePassed() + level.scr_dom_secured_all_bonus_time * 1000;
		}
	} else {
...
giveFlagCaptureXP( touchList )
{
	...
	for ( index = 0; index < players.size; index++ )
	{
		...
		touchList[players[index]].player.pers["bp_stats_domFlagsCaptured"]++;
	}
}
...
updateDomScores()
{
	...
	while ( !level.gameEnded )
	{
		...
		if ( isDefined( level.securedAllBonusTime ) && level.securedAllBonusTime <= openwarfare\_timer::getTimePassed() ) {
