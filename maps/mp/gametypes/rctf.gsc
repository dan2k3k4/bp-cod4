...
main()
{
	...
	level.scr_ctf_ctfmode = getdvarx( "scr_ctf_ctfmode", "int", 1, 0, 1  );
...
onStartGameType()
{
	...
	if ( nativeCTF ) {
		...
		level.origins["allies"] = getOriginFromBombZone( "sab_bomb_axis" );
		level.origins["axis"] = getOriginFromBombZone( "sab_bomb_allies" );
	}
	...
	level.spawn_allies = getentarray( "mp_" + spawnType + "_spawn_" + level.axisAssets, "classname" );
	level.spawn_axis = getentarray( "mp_" + spawnType + "_spawn_" + level.alliesAssets, "classname" );
	level.spawn_allies_start = getentarray("mp_" + spawnType + "_spawn_" + level.axisAssets + "_start", "classname" );
	level.spawn_axis_start = getentarray("mp_" + spawnType + "_spawn_" + level.alliesAssets + "_start", "classname" );
...
onPickupFailed( player )
{
	...
	if ( level.scr_ctf_flag_carrier_can_return == 1 ) {
		self flagReturned( player );
	}
}
...
resetObjectiveWaypoints( flagAtHome )
{
	...
	if ( flagAtHome ) {
		...
		level.zones[self.ownerTeam] maps\mp\gametypes\_gameobjects::allowUse( "friendly" );
		...
		level.zones[self.ownerTeam] maps\mp\gametypes\_gameobjects::allowUse( "none" );
	}
}
