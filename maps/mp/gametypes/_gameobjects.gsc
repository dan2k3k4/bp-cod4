...
main(allowed)
{
	...
	for(i = 0; i < entitytypes.size; i++)
	{
		if(isdefined(entitytypes[i].script_gameobjectname))
		{
			...
			/*/ Print map assets
			toLog = "Objectname: "+entitytypes[i].script_gameobjectname;
			if ( isDefined( entitytypes[i].targetname ) )
				toLog += " -> Targetname: " + entitytypes[i].targetname;

			if (isDefined( entitytypes[i].classname ) )
				toLog += " -> Classname: " + entitytypes[i].classname;
...
pickupObjectDelayTime( timeToWait )
{
	...
	delayEnds = openwarfare\_timer::getTimePassed() + timeToWait * 1000;
	while ( delayEnds > openwarfare\_timer::getTimePassed() )
		wait (0.2);
