...
main()
{
	...
	level.scr_ctf_ctfmode = getdvarx( "scr_ctf_ctfmode", "int", 1, 0, 2  );
...
onPrecacheGameType()
{
	...
	precacheString( &"BP_CTF_FLAG_DEFENDED_BY" );
	precacheString( &"BP_CTF_ENEMY_FLAG_DEFENDED_BY" );
}
...
onPickup( player )
{
	...
	if ( playerTeam != self.ownerTeam ) {
		thread printAndSoundOnEveryone( playerTeam, getOtherTeam( playerTeam ), &"MP_ENEMY_FLAG_TAKEN_BY", &"MP_FLAG_TAKEN_BY", "mp_war_objective_lost", "mp_lose_flag", player );
...
onDrop( player )
{
	...
	if ( isDefined( player ) ) {
		...
		thread printAndSoundOnEveryone( self.ownerTeam, getOtherTeam( self.ownerTeam ), &"OW_FLAG_DROPPED_BY", &"MP_ENEMY_FLAG_DROPPED_BY", "mp_war_objective_taken", "mp_enemy_obj_returned", player );
		...
		thread printAndSoundOnEveryone( self.ownerTeam, getOtherTeam( self.ownerTeam ), &"OW_FLAG_DROPPED", &"OW_ENEMY_FLAG_DROPPED", "mp_war_objective_taken", "mp_enemy_obj_returned", "" );
...
onReset( player )
{
	...
	// Play the corresponding sounds for players
	if ( isDefined( player ) ) {
		thread printAndSoundOnEveryone( self.ownerTeam, getOtherTeam( self.ownerTeam ), &"MP_FLAG_RETURNED_BY", &"OW_ENEMY_FLAG_RETURNED_BY", "mp_enemy_obj_taken", "mp_capture_flag", player );
		...
		thread printAndSoundOnEveryone( self.ownerTeam, getOtherTeam( self.ownerTeam ), &"MP_FLAG_RETURNED", &"MP_ENEMY_FLAG_RETURNED", "mp_enemy_obj_taken", "mp_capture_flag", "" );
...
Checks if the victim was killed within 10 meters of the enemy flag and gives the score for defending the flag
to the attacker
=============
*/
onPlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if ( isPlayer( attacker ) && self.pers["team"] != attacker.pers["team"] ) {
		...
		// 394 units = 10 meters
		if ( distanceToEnemyFlag <= 394 ) {
			attacker thread [[level.onXPEvent]]( "defend" );
			maps\mp\gametypes\_globallogic::givePlayerScore( "defend", attacker );
			
			level notify ("ctf_flag_defended", attacker);
			
			enemy = getOtherTeam(attacker.pers["team"]);
			
			printOnTeamArg(&"BP_CTF_FLAG_DEFENDED_BY", attacker.pers["team"], attacker.name);
			printOnTeamArg(&"BP_CTF_ENEMY_FLAG_DEFENDED_BY", enemy, attacker.name);
		}
	}
}
...
set_sprint()
{
	if(!isDefined(self.isFlagCarrier)) return;
	
	while(self.isFlagCarrier == true)
	{
		if(self.isFlagCarrier == false) return;
		if(isDefined(self))
...
unset_sprint()
{
	if(!isDefined(self.isFlagCarrier) || (self.isFlagCarrier == false))
