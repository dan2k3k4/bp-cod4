...
onBeginUse( player )
{
	if ( self maps\mp\gametypes\_gameobjects::isFriendlyTeam( player.pers["team"] ) )
	{
		...
		{
			for(i = 0; i < level.ddBombModel.size; i++) //foreach ( bomb in level.ddBombModel )
			{
				bomb = level.ddBombModel[i];
				if ( !isDefined( bomb ) )
					continue;
...
bombHandler( player, destType )
{
	...
	if ( destType == "explode" )
	{
		...
		{
			for(i = 0; i < level.players.size; i++) { //foreach ( splashPlayer in level.players )
				//if ( isDefined( player.pers["team"] ) && player.pers["team"] == team && isdefined( player.pers["class"] ) )
				splashPlayer = level.players[i];
