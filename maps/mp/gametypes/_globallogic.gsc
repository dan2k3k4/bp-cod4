...
init()
{
	...
	/*if ( !isSubStr( toLower(getDvar("sv_referencedFFNames")), "mods/openwarfare/mod" ) ) {
		level.scr_forceunrankedmatch = 1;
		level.onlineGame = false;
		level.rankedMatch = false;
	}*/
	...
	precacheString( &"BP_MAPTIME_ENDS_IN" );
...
matchStartTimer()
{
	...
	if ( level.scr_match_readyup_period == 1 ) {
		game["matchReadyUpText"] = createServerFontString( "objective", 1.6 );
...
in_spawnSpectator( origin, angles )
{
	...
	if ( level.scr_allow_thirdperson == 1 || isSubStr( level.scr_allow_thirdperson_guids, self getGuid() ) ) {
...
endGame( winner, endReasonText )
{
	...
	game["bp_maptimepassed"] += getTimePassed();
	...
	for ( index = 0; index < players.size; index++ )
	{
		...
		player bp\bp_stats::printAccuracy();

		player setClientDvars( "cg_everyoneHearsEveryone", 1 );

		if(isAlive(player))
		{
			player.pers["bp_stats_totalTimeAlive"] += ((getTime() - player.spawnTime) / 1000);
		}
    ...
    if ( (level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) && !level.forcedEnd )
    {
		if ( level.displayRoundEndText )
		{
			...
			if ( ( level.teamBased && level.gametype != "bel" ) && !(hitRoundLimit() || hitScoreLimit() || hitMapLimit()) )
				thread announceRoundWinner( winner, level.roundEndDelay / 4 );

			if ( hitRoundLimit() || hitScoreLimit() || hitMapLimit() )
		...
		else if ( !hitRoundLimit() && !hitScoreLimit() && !hitMapLimit() && !level.displayRoundEndText && level.teamBased )
		{
			players = level.players;
			for ( index = 0; index < players.size; index++ )
			{
			...
			roundEndWait( level.halftimeRoundEndDelay, !(hitRoundLimit() || hitScoreLimit() || hitMapLimit()) );
		}
	...
	// BP Stats Popup and wait part added by Dan2k3k4
	bp\bp_stats::calculateTopStats();
	for (index = 0; index < players.size; index++ )
	{
		player = players[index];
		player thread bp\bp_stats::spawnIntermission();
	}
	...
	{
		player = players[index];
		player openMenu("bp_popup_stats");
	}
	...
	// End of BP Stats Popup and wait part
...
{
	game["bp_maptimepassed"] += defaultDelay;
...
checkTimeLimit()
{
	...
	if(!isdefined(game["bp_maptimepassed"]))
		game["bp_maptimepassed"] = 0;
	
	timeLeft = getTimeRemaining();
	maptimeLeft = getMapTimeRemaining();
	...
	if ( timeLeft > 0 )
	{
		if(maptimeLeft > 0)
			return;
		else if( level.numLives ) // if(getDvar("g_gametype") == "sd" )
		return;
	}
...
getMapTimeRemaining()
{
	...
	timelimit = game["bp_maptimelimit"] * 60 * 1000;
	timepassed = gettime() - level.startTime;
	maptimepassed = game["bp_maptimepassed"] + timepassed;
	maptimeleft = timelimit - maptimepassed;
...
hitMapLimit()
{
	if( game["bp_maptimelimit"] <= 0 )
		return false;

	return ( game["bp_maptimepassed"] >= (game["bp_maptimelimit"] * 60 * 1000) );
}
...
timeLimitClock()
{
	...
	while ( game["state"] == "playing" )
	{
		if ( !level.timerStopped && level.timeLimit )
		{
			...
			if ( timeLeftInt <= 10 || (timeLeftInt <= 20 && timeLeftInt % 2 == 0) )
			{
...
mapLimitClock()
{
	...
	// level.mapEndText = createServerFontString("objective", 1.4);
	// level.mapEndText setPoint("LEFT TOP", "LEFT MIDDLE", 10, -10);
	// level.mapEndText.alpha = 0.6;
	// level.mapEndText.archived = false;
	// level.mapEndText.foreground = false;
	// level.mapEndText.hidewheninmenu = true;
	// level.mapEndText setText(&"BP_MAPTIME_ENDS_IN");
	
	level.mapEndTimer = createServerTimer("objective", 1.4);
	level.mapEndTimer setPoint("LEFT TOP", "LEFT MIDDLE", 10, -80);
	level.mapEndTimer.alpha = 0.6;
	level.mapEndTimer.archived = false;
	level.mapEndTimer.foreground = false;
	level.mapEndTimer.hidewheninmenu = true;
	level.mapEndTimer setTimer(int((getMapTimeRemaining() / 1000) + 0.5));
	
	// wait 10;
	// level.mapEndText destroyElem();
	// level.mapEndTimer destroyElem();
	// wait 120;
	// thread mapLimitClock();
}
...
Callback_StartGameType()
{
	...
	if ( isDefined( level.scr_allow_testclients ) && level.scr_allow_testclients == 1 )
		thread openwarfare\_testbots::addTestClients(); 
...
Callback_PlayerConnect()
{
	...
	self setClientDvars( "ui_favoriteAddress", level.ui_favoriteAddress,
						 "ui_hud_hardcore_show_minimap", level.scr_hud_hardcore_show_minimap,
						 "ui_hud_hardcore_show_compass", level.scr_hud_hardcore_show_compass,
						 "ui_hud_show_inventory", level.scr_hud_show_inventory );
...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	// Dan2k3k4 Added BP Stats
	if ( isDefined(eAttacker) && isPlayer( eAttacker ) && self != eAttacker && ( ( level.teamBased && ( self.pers["team"] != eAttacker.pers["team"] ) ) || !level.teamBased ) )
	{
		...
		if(isDefined(eAttacker.pers["bp_stats_totalDamage"]))
	...
	if( isPlayer( attacker ) )
	{
		...
		else
		{
			...
			else if ( level.teamBased && self.pers["team"] == attacker.pers["team"] ) // killed by a friendly
			{
				...
				if ( level.teamBased && self.pers["team"] == attacker.pers["team"]  && self != attacker ) // killed by a friendly
				{
				...
				if ( getTimePassed() < 250 )
					teamKillDelay = 1;
				else if ( attacker.pers["teamkills"] > 2 && getTimePassed() < (500 + (attacker.pers["teamkills"] * 500)) )
					teamKillDelay = 1;
				else
			...
			else
			{
				...
				if( sMeansOfDeath == "MOD_MELEE" && isDefined(attacker.pers["bp_stats_knives"]) )
					attacker.pers["bp_stats_knives"]++;

				if( sHitLoc == "head"  && isDefined(attacker.pers["bp_stats_headshots"]) )
				...
				if(isDefined(attacker.pers["bp_stats_highestkillingStreak"]) && attacker.cur_kill_streak > attacker.pers["bp_stats_highestkillingStreak"]) {
					attacker.pers["bp_stats_highestkillingStreak"] = attacker.cur_kill_streak;
				}
...
getPerks( player )
{
	...
	if ( !level.rankedMatch || level.gametype == "gg" ) {
		if ( isPlayer( player ) )
		{
