...
mayDropWeapon( weapon )
{
	...
	// [0.0.3] Handling weapondrops for modwarfare
...
watchGrenadePickup( grenadeType )
{
	...
	while(1)
	{
		...
		if ( ( grenadeType == "frag_grenade_mp" || grenadeType == "frag_grenade_short_mp" ) && isDefined( player.hasMartyrdom ) && player.hasMartyrdom && !player.hasFragsForMartyrdom ) {
...
watchCurrentFiring( curWeapon )
{
	...
	weapclass = weaponClass(curWeapon);
	if(weapclass == "spread") 
		self.pers["bp_stats_shotsFired"] += shotsFired * 8;
	else
		self.pers["bp_stats_shotsFired"] += shotsFired;
