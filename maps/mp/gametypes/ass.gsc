...
main()
{
	...
	level.scr_ass_model_enable = getdvarx( "scr_ass_model_enable", "int", 0, 0, 1 );
	level.scr_ass_dualpistols = getdvarx( "scr_ass_dualpistols", "int", 1, 0, 1 );
	//level.scr_ass_sniper = getdvarx( "scr_ass_sniper", "int", 0, 0, 1 );
...
onStartGameType()
{
	...
	nativeSAB = isDefined( getEnt( "sab_bomb_axis", "targetname" ) );
	...
	if ( nativeASS ) {
		...
		// Let's get the trigger origins of the SD defender's bombsite before we get rid of all the map assets
		level.extractionZoneOrigin = getOriginFromSDBombZone( "_a" );
	...
	if ( nativeASS ) {
		...
		level.spawn_attackers_start = getentarray( "mp_sd_spawn_attacker", "classname" );
		level.spawn_defenders_start = getentarray( "mp_sd_spawn_defender", "classname" );
	}
...
/*
=============
getOriginFromSDBombZone

Get the origin of an entity to be used in case entities need to be manually created
=============
*/
getOriginFromSDBombZone( label )
{
...
onUse( player )
{
	...
	if ( isPlayer( player ) && player.isVIP ) {
		...
		level notify("vipextracted", player);
...
pickVIP()
{
	...
	level notify("vippicked", level.playerVIP);
	...
	if ( level.scr_ass_dualpistols )
		level.playerVIP thread dualPistols();
	else
...
removeInvalidWeapons()
{
	...
	if ( level.scr_ass_force_vip_handgun != "" ) {
		...
		for( i = 0; i < weaponsList.size; i++ )	{
			...
			if ( weaponClass( currentWeapon ) == "pistol" && ( weaponClass( level.scr_ass_force_vip_handgun ) != "pistol" || currentWeapon == "binoculars_mp" ) )
				continue;
				
			self takeWeapon( currentWeapon );
		}
		
		// Player will get this weapon
		handGun = level.scr_ass_force_vip_handgun;		
		
	} else {
		handGun = "";
		
		// Remove everything except pistol and grenades
		for( i = 0; i < weaponsList.size; i++ )	{
			currentWeapon = weaponsList[i];
			...
			if ( weaponClass( currentWeapon ) == "pistol" ) {
				if ( currentWeapon != "binoculars_mp" ) {
					self takeWeapon( currentWeapon );
...
dualPistols()
{
	...
	for( i = 0; i < weaponsList.size; i++ )	{
		currentWeapon = weaponsList[i];
		// Remove all primary weapon and seconday weapons
		if ( weaponClass( currentWeapon ) != "grenade" ) {
			self takeWeapon( currentWeapon );
		}
	}
	
	// Give the player two pistols
	handGun1 = "deserteaglegold_mp";
	handGun2 = "deserteagle_mp";
	self giveWeapon( handGun1 );
	self giveWeapon( handGun2 );
	
	// Give maximum amount of ammo for the weapons and switch to first one
	self giveMaxAmmo( handGun1 );
	self giveMaxAmmo( handGun2 );
	self switchToWeapon( handGun1 );
}
