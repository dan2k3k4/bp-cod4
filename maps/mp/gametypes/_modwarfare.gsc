...
init()
{
	...
	initWeaponData( "kar98", "assault", "axis" );
	...
	//TODO: game["axis_perk_rpg_mp_limit"] = getdvarx( "axis_perk_rpg_mp_limit", "int", 64, 0, 64 );
	game["perk_c4_mp_limit"] = getdvarx( "perk_c4_mp_limit", "int", 64, 0, 64 );
	//TODO: game["perk_extraammo_mp_limit"] = getdvarx( "perk_extraammo_mp_limit", "int", 64, 0, 64 );
	//TODO: game["axis_perk_extraammo_mp_limit"] = getdvarx( "axis_perk_extraammo_mp_limit", "int", 64, 0, 64 );
...
updateAvailableClasses()
{
	...
	self setClientDvars(
		...
		"allies_allow_assault", ( game["allies_assault_limit"] > game["class_counts"]["allies_assault"] || playerClass == "assault" ),
		"allies_allow_specops", ( game["allies_specops_limit"] > game["class_counts"]["allies_specops"] || playerClass == "specops" ),
		"allies_allow_heavygunner", ( game["allies_heavygunner_limit"] > game["class_counts"]["allies_heavygunner"] || playerClass == "heavygunner" ),
		"allies_allow_demolitions", ( game["allies_demolitions_limit"] > game["class_counts"]["allies_demolitions"] || playerClass == "demolitions" ),
		"allies_allow_sniper", ( game["allies_sniper_limit"] > game["class_counts"]["allies_sniper"] || playerClass == "sniper" ),

		
		"axis_allow_assault", ( game["axis_assault_limit"] > game["class_counts"]["axis_assault"] || playerClass == "assault" ),
		"axis_allow_specops", ( game["axis_specops_limit"] > game["class_counts"]["axis_specops"] || playerClass == "specops" ),
		"axis_allow_heavygunner", ( game["axis_heavygunner_limit"] > game["class_counts"]["axis_heavygunner"] || playerClass == "heavygunner" ),
		"axis_allow_demolitions", ( game["axis_demolitions_limit"] > game["class_counts"]["axis_demolitions"] || playerClass == "demolitions" ),
...
setLoadoutForClass( classType )
{
	...
	if ( !isDefined( self.pers[classType] ) || changedTeam )
	{
		...
		if ( isSubstr( "mp44", self.pers[classType]["loadout_primary"] ) || isSubstr( "kar98", self.pers[classType]["loadout_primary"] ) ) {
		...
		switch ( subTokens[0] )
		{
			case "loadout_primary":
				if ( isWeaponAllowed( self.class, subTokens[1], playerTeam ) && self verifyWeaponChoice( subTokens[1], self.class ) )
				{
					...
					if ( subTokens[1] == "mp44" || subTokens[1] == "kar98" )
					{
			...
			case "loadout_secondary_attachment":				
				if ( subTokens[0] == "loadout_primary_attachment" && ( self.pers[self.class]["loadout_primary"] == "mp44" || self.pers[self.class]["loadout_primary"] == "kar98" ) )
				{
					self.pers[self.class]["loadout_primary_attachment"] = "none";
					self setClientDvar( "loadout_primary_attachment", "none" );
