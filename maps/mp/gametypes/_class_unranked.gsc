...
giveLoadout( team, class )
{
	...
	{
		case "claymore_mp":
			ammoCount = getdvarx( "scr_claymore_ammo_count", "int", 2, 1, 24 );
			break;
		case "rpg_mp":
			ammoCount = getdvarx( "scr_rpg_ammo_count", "int", 2, 1, 24 );
			break;
		case "c4_mp":
			ammoCount = getdvarx( "scr_c4_ammo_count", "int", 2, 1, 24 );
			break;
		default:
			ammoCount = 2;
			break;
	...
	if ( maps\mp\gametypes\_modwarfare::isWeaponAllowed( "all", "frag_grenade", "all" ) )
	{
		...
		if ( isdefined ( grenadeCount ) && grenadeCount ) {
			self.primarynade = self.pers[class]["loadout_pgrenade"] + "_mp"; //"frag_grenade_mp";
