...
onPlayerKilled( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	...
	if ( sMeansOfDeath != "MOD_FALLING" && sMeansOfDeath != "MOD_TRIGGER_HURT" && sMeansOfDeath != "MOD_SUICIDE" ) {
		...
		self.freezeTag["origin"] = self getOrigin();
...
startHeatTransfer( frozenPlayer, triggerRadius )
{
	...
	// Give score to the player every level.unfreezeUnitsPerPoint units of health given
	if ( isDefined( self ) ) {
		unfreezeIcon.alpha = 0;
		totalPoints = int( self.freezeTag["healthgiven"] / level.unfreezeUnitsPerPoint );
		if ( totalPoints > 0 ) {
			self.freezeTag["healthgiven"] = self.freezeTag["healthgiven"] - ( totalPoints * level.unfreezeUnitsPerPoint );
			self giveUnfreezeScore( totalPoints );
		}
		self.freezeTag["transfer"] = false;
	}
...
giveUnfreezeScore( totalPoints )
{
	// Every 10 points (complete life restore) will give the player an assist point
	if ( !isDefined( self.freezeTag["assist"] ) )
		self.freezeTag["assist"] = 0;
		
	self.freezeTag["assist"] += totalPoints;
	completeAssists = int( self.freezeTag["assist"] / level.unfreezeUnitsPerPoint );
	if ( completeAssists > 0 ) {
		self maps\mp\gametypes\_globallogic::incPersStat( "assists", completeAssists );
		self.assists = self maps\mp\gametypes\_globallogic::getPersStat( "assists" );
		self.freezeTag["assist"] = self.freezeTag["assist"] - ( completeAssists * level.unfreezeUnitsPerPoint );
	}
	
	// Give the player the score
	totalPoints *= level.scr_ftag_unfreeze_score;
	self maps\mp\gametypes\_rank::giveRankXP( "unfreeze", totalPoints );
	self.pers["score"] += totalPoints;
	self maps\mp\gametypes\_persistence::statAdd( "score", ( self.pers["score"] - totalPoints ) );
...
addUnfreezingBudy( budy )
{
	...
	i = 0;
	while ( i < self.freezeTag["unfreezebudies"].size && self.freezeTag["unfreezebudies"][i].playerEntity != budyEntity )
		i++;
	
	// If we couldn't find the entity we'll just add a new element
	if ( i == self.freezeTag["unfreezebudies"].size ) {
